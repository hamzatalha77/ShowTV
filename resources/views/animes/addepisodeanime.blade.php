...
@section('content')
<form action="{{ route('anime_episodes.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
    @csrf

  <h3>Add New Episode For: {{ $season->season_name }} Number:{{ $season->season_number }} -- {{ $anime->anime_name }}</h3>
  <input type="text" name="views" value="0" hidden>
    <img class="image1" src="{{ $season->season_image }}">
<div class="con">
    <input  type="text" name="season_id" value="{{$season->id}}" class="form-control" id="user_name" hidden>
    <div class="mb-3">
